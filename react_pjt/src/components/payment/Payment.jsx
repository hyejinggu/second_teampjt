...
const Payment = () => {
    ...
    let selectedItem = location.state?.selectedItem;
    const quantity = location.state?.quantity;
    console.log(selectedItem)

    useEffect(() => {
        const storedProduct = JSON.parse(localStorage.getItem("product"));
        if (!selectedItem && storedProduct) {
            selectedItem = storedProduct;
        }
    }, []);
    ...
    const totalPrice = () => {
        const originalPr = selectedItem?.normalPr || 0;
        const salePr = originalPr - (originalPr * (selectedItem?.saleInfo || 0) / 100);
        const totalpr = salePr * (quantity || 0);
        return totalpr;
    };
